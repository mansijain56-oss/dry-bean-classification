...
@st.cache_data
def load_dataset():
    return pd.read_excel("Dry_Bean_Dataset.xlsx")
...
# ==========================================================
    ...
    try:
        ...
        with tab4:
            ...
            # Get test data and generate confusion matrix
            X = df[FEATURES]
            y = encoder.transform(df[TARGET])
            X_scaled = scaler.transform(X)

            y_pred = models[selected_cm_model].predict(X_scaled)
            ...
            report = classification_report(y, y_pred, target_names=encoder.classes_, output_dict=True)
